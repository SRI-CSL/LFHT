...
\jorge{I try to use the term ``arrays'' for each table copy and the
  term ``table'' for the whole hash table. Need to review this}
...
later. \todo{list limitations of the model (e.g., counters updates are
  considered atomic, no handle)}. For convenience, the arrays in our
...
\texttt{cur\_sz} represents the size of the new array. The value of
...
keeps track of several counters to monitorize thread's
activities\jorge{rename these counters!}:

\begin{itemize}
\item \texttt{pending}: number of threads at state \pcone.
\item \texttt{did\_not\_pay}: number of threads that added an entry
  without migrating entries from the old to the new array.
...
\pcode[\small]{
  ...
  \> $pending \incr 1$, $paid\_tax \incr 1$ \\
  \> $revenue \incr num\_to\_migrate$\\
  \> $state := \pcone$}
\end{minipage}
...
format and ran Sally on it. It turned out that Sally was not able to
find an inductive strengthening in a reasonable amount of time.
...
\jorge{I think we can split lemmas in the following groups: only about
  old (Lemmas~\ref{old_is_full}), only about new
  (Lemmas~\ref{new_cannot_be_assimilated},\ref{new_cannot_migrate}),
  relating old and new (Lemmas~\ref{old_and_new}), for multiple
  tables
  (Lemmas~\ref{empty_beyond_new},\ref{empty_beyond_non_full},\ref{num_to_migrate_and_num_entries},\ref{table_sizes},\ref{not_full_cannot_be_assimilated},\ref{assimilated_nothing_to_migrate},\ref{num_to_migrate_and_assimilated_in_old_tables},\ref{upper_global}),
  and thread related
  (Lemmas~\ref{one_thread_at_init_state_after_migration},\ref{pending},\ref{pending_lb},\ref{did_not_pay_ub},\ref{paid_tax0},\ref{paid_tax1},\ref{revenue1},\ref{revenue2},\ref{revenue3},\ref{posted0},\ref{posted1},\ref{out_of_mem}). Still need to categorize ~\ref{upper_strengthening1},\ref{upper_strengthening2},\ref{upper_strengthening3},\ref{upper},etc}

\begin{lemma}[Order between old and new array]
  \begin{equation*}
    \begin{array}{l}      
...
\begin{lemma}[After new arrray, all arrays must be empty]
  \begin{equation*}
\begin{array}{l}      
...
\begin{lemma}[If an array is not exceed the occupancy threaholds then all arrays after it  must be empty]
  \begin{equation*}
\begin{array}{l}        
...
\begin{lemma}[Number of migrations cannot exceed the number of entries]
  \begin{equation*}
\begin{array}{l}        
...
\begin{lemma}[Size of each array]
  \begin{equation*}
\begin{array}{l}        
...
\begin{lemma}[The new array cannot be assimilated]
  \begin{equation*}
\begin{array}{l}        
...
\begin{lemma}[The new array cannot have any entry to migrate]
  \begin{equation*}
\begin{array}{l}      
...
\begin{lemma}[The old array, if different from new, always exceeds the occupancy threshold]
  \begin{equation*}
\begin{array}{l}        
...
\begin{lemma}[If an array is not full then it cannot be assimilated]
  \begin{equation*}
\begin{array}{l}      
...
\begin{lemma}[If an array is assimilated then there is no more entries to migrate]
  \begin{equation*}
\begin{array}{l}        
...
\begin{lemma}[Relationship between number to migrate and assimilated flag in the old arrays]
  \begin{equation*}
    \begin{array}{l}
...
\begin{lemma}[At least one thread must be at the initial state after migration finished]
  \begin{equation*}
    \begin{array}{l}
...
\begin{lemma}[Upper bound for did\_not\_pay]
  \begin{equation*}
\begin{array}{l}        
  did\_not\_pay < P
\end{array}
...
\begin{lemma}[Exact bound for paid\_tax before having copies]
  \begin{equation*}
\begin{array}{l}        
...
\begin{lemma}[Lower bound for paid\_tax]        
  \begin{equation*}
\begin{array}{l}
  paid\_tax \geq 0
\end{array}
...
\begin{lemma}[Exact bound for revenue if table grew]
  \begin{equation*}
\begin{array}{l}        
...
\begin{lemma}[Upper bound for revenue if table grew]
  \begin{equation*}
\begin{array}{l}        
...
\begin{lemma}[Relationship between number to migrate in the old array, revenue and paid\_tax]
    \begin{equation*}
\begin{array}{l}      
...
\begin{lemma}[Lower bound for posted]
    \begin{equation*}
\begin{array}{l}      
  posted \geq 0
\end{array}
...
\begin{lemma}[Relationship between assimilated in the old array, posted, pending, paid\_tax and did\_not\_pay]
  \begin{equation*}
    \begin{array}{l}      
...
\begin{lemma}[Relationship between number of entries in the new array, paid\_tax, and posted]
  \begin{equation*}
    \begin{array}{l}      
...
\begin{lemma}[Relationship between number of entries in the new array, number to migrate in the old,  paid\_tax, posted]
\begin{equation*}
\begin{array}{l}  
...
\begin{lemma}[Exact bound of the number of entries for the new array]
  \begin{equation*}
    \begin{array}{l}  
      old < new \Rightarrow \\
...
\begin{lemma}[Upper bound of the number of entries for all arrays]
  \begin{equation*}
\begin{array}{l}      
...
\begin{lemma}[Upper bound of the number of entries for old arrays]
  \begin{equation*}
\begin{array}{l}    
...
the Sal language~\cite{bensalem2000overview} and used the SAL
...
\begin{table}[t]
  \begin{center}
    \begin{tabular}{|c|l|l|}
      ...
      \hline 
    \end{tabular}
    \caption{Proving key invariant (and all auxiliary lemmas) with SAL up to 7 threads}
    \label{sal-results}
    \end{center}
\end{table}
...
maximum number of allocated arrays is three. If this limit is exceeded
